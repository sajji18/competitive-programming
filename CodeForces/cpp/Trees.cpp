...
ll dfs (int node, int flag, int parent, vector <int> adj[], vector <int> &l, vector <int> &r, vector <vector <ll>> &dp) {
    if (dp[node][flag] != -1) return dp[node][flag];
    ll nval = (flag) ? r[node] : l[node];
    ll curr = 0;
    for (auto &child : adj[node]) {
        if (child != parent) {
            curr += max(
                    abs(nval - l[child]) + dfs(child, 0, node, adj, l, r, dp),
                    abs(nval - r[child]) + dfs(child, 1, node, adj, l, r, dp)
                );
        }
    }
    return dp[node][flag] = curr;
}
...
void solve() {
    ...
    ll ans = max(dfs(1, 0, -1, adj, l, r, dp), dfs(1, 1, -1, adj, l, r, dp));
    cout << ans << "\n";
}
